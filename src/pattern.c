...
static inline uint8_t cts_hex_nibble_val(char p_hex_nibble) {
   if (p_hex_nibble >= '0' && p_hex_nibble <= '9') {
      return (uint8_t) (p_hex_nibble - '0');
   } else if (p_hex_nibble >= 'A' && p_hex_nibble <= 'F') {
      return (uint8_t) (p_hex_nibble - 'A' - 10);
   } else if (p_hex_nibble >= 'a' && p_hex_nibble <= 'f') {
      return (uint8_t) (p_hex_nibble - 'a' - 10);
   }
}
 ...
 */
static inline uint8_t cts_is_hex_char(char p_test) {
   return (p_test >= '0' && p_test <= '9') ||
          (p_test >= 'A' && p_test <= 'F') ||
          (p_test >= 'a' && p_test <= 'f');
}

struct s_rule* cts_parse_rule(char* p_rule_spec) {
   size_t const spec_len = strlen(p_rule_spec);

   // Allocate a buffer to store the pattern in while reading it.
   // spec_len / 2 should represent the maximum theoretical size of the
   // compiled pattern.
   size_t pattern_buf_size = sizeof(uint8_t) * ceil(((double) spec_len / 2.0f));
   uint8_t* pattern_buffer = malloc(pattern_buf_size);

   printf("pattern buffer size: %zu\n", pattern_buf_size);
   ...
   // Read in pattern data
   {
      do {
         ...
         if((tok == '?' && pattern_length > 0) || tok == 0) {
            ...
            break;
         } else if(cts_is_hex_char(tok)) { // The high nibble is present
            if(tok_idx < spec_len) { // Is another character available
               ...
               if(cts_is_hex_char(tok)) { // Expect low nibble
                  *(pattern_buffer + pattern_length++) = cts_hex_nibble_val(tok) << 4 | cts_hex_nibble_val(next_tok);
                  ...
                  fprintf(stderr, "Hanging character `%c` (pos %zu in pattern `%s`) needs a low nibble but the next character (`%c`) is not a hexadecimal digit\n", tok, tok_idx - 2, p_rule_spec, next_tok);
                  goto cleanup_retn;
               }
            } else {
               fprintf(stderr, "Hanging character `%c` (pos %zu in pattern `%s`) needs a low nibble but EOS has been reached\n", tok, tok_idx - 1, p_rule_spec);
               goto cleanup_retn;
            }
         }
   ...
   if(tok_idx > spec_len) {
      fprintf(stderr, "Pattern string iterator overrun (at %zu but string is %zu bytes)\n", tok_idx, spec_len);
      goto cleanup_retn;
   }
